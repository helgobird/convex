...
поступающих точек плоскости и вычисляющую её периметр, площадь и количество
её вершин, расположенных в 1-окрестности прямой, заданной двумя различными
точками плоскости. Решение должно быть индуктивным, что означает определение
выпуклой оболочки и вычисление её характеристик сразу после поступления очередной
точки с использованием методов теории индуктивных функций.
...
- Вспомогательные классы:
    ...
    - `Data` — две точки, задающие прямую
    - `Line` — прямая, заданная своим общим уравнением
- Основные классы:
...
- Файлы проекта:
    ...
    - `data.py` — реализация класса `Data`
    - `lines.py` — реализация класса `Line`
    ...
    - `tests/test_lines.py` — тесты к классу `Line`
    - `tests/test_data.py` — тесты к классу `Data`
...
### Алгоритм решения задачи модификации

В класс `R2Point` была добавлена функция `in_neighbourhood`,
определяющая, принадлежит ли данная точка 1-окрестности заданной
прямой. При индуктивном пересчете характеристик выпуклой оболочки
для каждой освещенной вершины определяется, принадлежит ли она
1-окрестности заданной прямой, и, если принадлжит, число вершин
лежащих в 1-окрестности прямой уменьшается на единицу при удалении
